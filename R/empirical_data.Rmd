...
new_stom_df <- read_csv("../data/stom_data_light.csv") %>%
...
fle_h <- read_csv("../data/flounder_cpue_length_historical.csv") %>%
  filter(sub_div %in% c(25:28) & 
           year >= 1978 & 
           year <= 2015 & 
           quarter %in% c(1,4) &
...
#   group_by(year, quarter, species, length_class) %>% #year_cat
...
dens_sum <- cod_fle_size %>%
  ...
  summarise(tot_density = sum(length_bio_dens)) %>%
  ungroup()
...
stom_count_data <- new_stom_df %>% filter(!is.na(species_group) & 
                  predator_length_cm > 10) %>%  
  mutate(year_agg = as.character(cut(year, year_brackets, dig.lab = 10))) %>%
...
gg_stom
#ggsave(gg_stom, filename = "../fig/stom_cont.pdf", height = 8, width = 15)
```
...
stom_count_data <- new_stom_data %>% 
  ...
  filter(prey_weight_tot > 0 & prey_weight_tot < 10000) %>% 
...
## NEW Horn-Morisita Distance index
