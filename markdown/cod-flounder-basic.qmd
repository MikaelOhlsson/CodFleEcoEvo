...
To implement the model of @eq-m, we first load some packages:

```{r}
library(tidyverse) # Efficient data manipulation and plotting
library(patchwork) # Joining plots in a grid
library(nleqslv)   # Nonlinear equation solver (for computing equilibria)
```

We then define a function to compute $\text{d}\mu / \text{d}t$:

```{r}
# Rate of change of cod body mass, as a function of the body mass m itself
# plus model parameters
dmdt <- function(m, mF = 0, h2 = 0.5, rho = 5, s = 0.5, sF = 0.5,
                 theta = 5, zstar = 1, eta = 0, phi = 0, tau = 0.5,
                 kappa = 2, zeta = 1, nu = 1.5, Fpop = 6, w = 0.33,
                 alpha0 = 1/sqrt(2), alphaI = 1) {
...
Creating an illustration of the body size distributions before fishing:
...
body_sizes_before <-
  ...
  pivot_longer(cols = c(flounder, cod),
               names_to = "species",
               values_to = "density") |>
  mutate(density = ifelse(density < 1e-4, NA, density)) |>
...
Creating an illustration of the body size distributions after fishing:
...
The body size-dependent rate of change for cod body size, without fishing:

```{r}
# Small table of labels, for the next plot
equilib_nofish <-
  tibble(
    # Start searching for equilibria from these points:
    x_approx = c(-1, 0, 1),
    y = c(0, 0, 0),
    sh = as_factor(c("small cod","flounder","large cod")),
    lab = c("a", "b", "c")
  ) |>
  # Solve for exact equilibrium points:
  mutate(x = map_dbl(x_approx, \(eta) nleqslv(eta, dmdt)$x))
...
bodysize_eqb_nofish <-
  ...
  geom_point(data = equilib_nofish, aes(x,y, shape = sh, fill = sh),
             size  = 3) +
  scale_shape_manual(values = c(23, 21, 22)) +
  scale_color_manual(
    values = c("cornflowerblue", "darkseagreen", "cornflowerblue")
  ) +
  scale_fill_manual(
    values = c("cornflowerblue", "darkseagreen", "cornflowerblue")
  ) +
...
The body size-dependent rate of change for cod body size, with fishing:
...
equilib_fish <-
  ...
  # Solve for exact equilibrium points:
  mutate(x = case_when(
    # For label "a", we properly solve:
    lab == "a" ~ nleqslv(x_approx, \(m) dmdt(m, eta = 2))$x,
    # For label "b" there is no equilibrium point, so replace with
    # value from the no-fishing scenario:
    lab == "b" ~ equilib_nofish |> filter(lab == "b") |> pull(x)
  ))
